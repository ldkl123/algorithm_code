...
def solution(priorities, location):
    ...
    while True:
        ...
        J = actual_list.get()           # Get first print which is located at front
        prior = lst[0][1]               # Get high priority
        
        if J < prior:               # If first printer's priority is lower than high priority
            actual_list.put(J)      # put printer back to the wait list
        else:
            lst.get()         
            answer+=1
...
# Reuslt: compile error, ther is not get() operation in lst(list)
